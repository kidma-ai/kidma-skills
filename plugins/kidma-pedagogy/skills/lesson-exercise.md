---
description: "Generate a branded 1-2 page Kidma lesson summary PDF for students. The summary distills the core concepts, key terms, processes, and takeaways from a lesson so that any student who missed the class can catch up quickly. Content is sourced from the lesson's slides (slides-builder) and/or the instructor lesson plan doc (lesson-to-word). Style follows Kidma brand guidelines exactly (Assistant font, brand colors, wave footer, RTL Hebrew). Output is a ready-to-distribute PDF file. Use this skill when the user asks to: - Create a \"סיכום שיעור\" (lesson summary) PDF - Generate a student handout / take-home sheet for a lesson - Build a catch-up document for absent students - Produce a reference card summarizing a Kidma lesson - Says anything like \"תן לי סיכום לשיעור\", \"צור דף סיכום\", \"סיכום לתלמיד\",    \"student summary\", \"lesson recap PDF\", \"דף מסכם לתלמיד\""
type: generation
depends_on:
  - kidma-plugins:brand
invokes: []
input_format: json
input_file: "/home/claude/summary_content.json"
input_schema: "./schemas/summary-content.schema.json"
output_format: pdf
output_file: "/mnt/user-data/outputs/lesson_summary.pdf"
runtime_tools:
  - bash
python_packages:
  - reportlab
...
- `kidma-plugins:brand skill` — colors, fonts, logos
...
Build the summary as a JSON object following the schema at `../schemas/summary-content.schema.json`. Key fields:

| Field | Type | Description |
|---|---|---|
| `lesson_title` | string | Full title, e.g. `"סיכום שיעור טקסט לדיבור"` |
| `lesson_subtitle` | string | Subtitle/topic line shown below title |
| `two_column` | bool | `true` splits sections into a 2-column grid |
| `sections[].title` | string | Section header text (Hebrew) |
| `sections[].color` | string | `vocab`/`understand`→Blue, `try`→Green, `create`→Pink, `process`/`default`→Dark |
| `sections[].type` | string | `bullets`, `steps`, `callout`, or `text` |
| `sections[].items` | array | `{term?, text}` objects — for `bullets` and `steps` types |
| `sections[].content` | string | Paragraph content — for `callout` and `text` types |
...
```json
{
  ...
  "lesson_subtitle": "תת-כותרת",
  "two_column": false,
  "sections": [
    { "title": "מושגי יסוד", "color": "vocab", "type": "bullets",
      "items": [{ "term": "מונח – English", "text": "הגדרה." }] },
    { "title": "💡 זכרו", "color": "default", "type": "callout",
      "content": "הבינה המלאכותית היא כלי חזק אך לא מושלם." }
  ]
}
```
...
- **`slides-builder`** — Source of slide content for summaries
- **`lesson-to-word`** — Source of instructor lesson plan content
- **`lesson-planner`** — Generates lesson content JSON (can be adapted for summaries)
- **`kidma-plugins:brand`** — Brand colors, fonts, logo rules
- **`kidma-plugins:overview`** — Full Kidma program context