...
invokes:
  ...
  - lesson-exercise
output_format: json
output_file: "/home/claude/lesson_content.json"
output_schema: "./schemas/lesson-content.schema.json"
---
...
3. **Read and follow the `lesson-to-word` skill** to produce the branded document

### JSON Schema

The output must follow the schema at `../schemas/lesson-content.schema.json`. Key top-level fields:

| Field | Type | Description |
|---|---|---|
| `lesson_number` | string | Lesson number in sequence |
| `title` | string | Lesson title (Hebrew) |
| `objectives` | object | `understanding`, `experimentation`, `creation` bullet arrays |
| `materials` | array | Required materials list |
| `learning_materials` | string | Instructor self-study links (free text) |
| `structure` | object | Five phases: `introduction`, `understanding`, `experimentation`, `creation`, `summary` — each with `description` + `bullets[]` |
| `dead_time` | string | Filler activities |
| `assessment` | string | How to evaluate success |
| `instructor_notes` | string | Tips for the instructor |
